// based on willgm/web-crypto-storage
...
export class CryptoStorage {
  ...
  async get(key: InputData): Promise<string | undefined> {
    const [
      [store, storeName, baseKey, salt, encryptIterations],
      hashKey,
      hashNonce,
    ] = await all([
      this.internalConfig,
  ...
  async set(key: InputData, value: InputData): Promise<void> {
    ...
    await all([
      store.put(storeName, cryptoValue, hashKey),
      store.put(storeName, nonce, hashNonce),
    ]);
  }
  ...
  async delete(key: InputData): Promise<void> {
    ...
    await all([
      store.delete(storeName, hashKey),
      store.delete(storeName, hashNonce),
    ]);
  }
